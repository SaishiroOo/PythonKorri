import sys
import sqlite3
import webbrowser
from PySide6.QtWidgets import (QDateEdit, QComboBox, QLineEdit, QPushButton, QApplication, QVBoxLayout, QDialog, QLabel, QHBoxLayout, QWidget)

conn = sqlite3.connect("important.db")
c = conn.cursor()
c.execute(
"""
CREATE TABLE IF NOT EXISTS personne
(
    id INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE,
    username text,
    prenom text,
    nom text,
    password text
            
)
""")

conn.commit()
conn.close()

class Connexion(QDialog):

    def __init__(self, parent=None):
        super(Connexion, self).__init__(parent)
        # Create widgets

        self.setWindowTitle('Connexion')
        self.label1 = QLabel("Welcome")
        self.label2 = QLabel("Log In or Create a new account")
              
        self.button = QPushButton("Log In")
        self.button2 = QPushButton("Create a new account")
        self.button3 = QPushButton("Exit")
        
        # Create layout and add widgets

        layoutVer = QVBoxLayout()

        layoutVer.addWidget(self.label1)
        layoutVer.addWidget(self.label2)
        layoutVer.addWidget(self.button)
        layoutVer.addWidget(self.button2)
        layoutVer.addWidget(self.button3)

        # Set dialog layout
        self.setLayout(layoutVer)

        # Add button signal to greetings slot
        self.button.clicked.connect(self.Login)
        self.button2.clicked.connect(self.Create)
        self.button3.clicked.connect(self.close)

    # Greets the user
    def Login(self):

        form.hide()
        form2.show()

    def Create(self):

        form.hide()
        form4.show()

class Login(QDialog):

    def __init__(self, parent=None):
        super(Login, self).__init__(parent)
        # Create widgets

        self.setWindowTitle('Login')
        self.label1 = QLabel("Log In")
        self.label2 = QLabel("Log In to your existing account")
        self.label3 = QLabel("Username")
        self.label4 = QLabel("Password")
        self.label5 = QLabel("Login ou mot de passe incorrect")
        self.label5.hide()

        self.edit1 = QLineEdit()
        self.edit2 = QLineEdit()

        self.edit2.setEchoMode((QLineEdit.Password))
        
        self.button = QPushButton("Log In")

        # Create layout and add widgets

        layoutVer = QVBoxLayout()

        layoutVer.addWidget(self.label1)
        layoutVer.addWidget(self.label2)
        layoutVer.addWidget(self.label3)
        layoutVer.addWidget(self.edit1)
        layoutVer.addWidget(self.label4)
        layoutVer.addWidget(self.edit2)
        layoutVer.addWidget(self.label5)
        layoutVer.addWidget(self.button)

        # Set dialog layout
        self.setLayout(layoutVer)

        # Add button signal to greetings slot
        self.button.clicked.connect(self.Login)

    # Greets the user
    def Login(self):

        conn = sqlite3.connect("important.db")
        c = conn.cursor()

        c.execute("""SELECT username FROM personne WHERE username= ? AND password = ?""", (self.edit1.text(), self.edit2.text()))

        if not c.fetchone():  # An empty result evaluates to False.
            self.label5.show()
        else:
            form2.hide()
            form3.show()
            self.label5.hide()

        conn.commit()
        conn.close()

        self.edit1.setText("")
        self.edit2.setText("") 

  

class SignUp(QDialog):

    def __init__(self, parent=None):
        super(SignUp, self).__init__(parent)
        # Create widgets

        self.setWindowTitle('Sign up')
        self.label1 = QLabel("Sign up")
        self.label2 = QLabel("Create new account now")
        self.label3 = QLabel("Username")
        self.label4 = QLabel("Password")
        self.label5 = QLabel("Confirm Password")
        self.label6 = QLabel("Champs incomplet ou mot de passe incorrect")
        self.label6.hide()

        self.edit1 = QLineEdit()
        self.edit2 = QLineEdit()
        self.edit3 = QLineEdit()   

        self.edit2.setEchoMode((QLineEdit.Password))
        self.edit3.setEchoMode((QLineEdit.Password))
        
        self.button = QPushButton("Sign up")

        # Create layout and add widgets

        layoutVer = QVBoxLayout()

        layoutVer.addWidget(self.label1)
        layoutVer.addWidget(self.label2)
        layoutVer.addWidget(self.label3)
        layoutVer.addWidget(self.edit1)
        layoutVer.addWidget(self.label4)
        layoutVer.addWidget(self.edit2)
        layoutVer.addWidget(self.label5)
        layoutVer.addWidget(self.edit3)
        layoutVer.addWidget(self.label6)
        layoutVer.addWidget(self.button)

        # Set dialog layout
        self.setLayout(layoutVer)

        # Add button signal to greetings slot
        self.button.clicked.connect(self.Sign)

    # Greets the user
    def Sign(self):

        if(self.edit1.text()=="" or self.edit2.text()=="" or self.edit3.text()==""):

            self.edit1.setText("")
            self.edit2.setText("") 
            self.edit3.setText("")
            self.label6.show()

        elif(self.edit2.text() == self.edit3.text()):

            conn = sqlite3.connect("important.db")
            c = conn.cursor()

            c.execute("""INSERT INTO personne (username, password)  VALUES(?, ?)""", (self.edit1.text(), self.edit2.text()))
            form4.hide()
            form5.show()
            self.label6.hide()

            conn.commit()
            conn.close()

            self.edit1.setText("")
            self.edit2.setText("")
            self.edit3.setText("")

class Submit(QDialog):

    def __init__(self, parent=None):
        super(Submit, self).__init__(parent)
        # Create widgets

        self.setWindowTitle('Submit')
        self.label1 = QLabel("Fill profile")
        self.label2 = QLabel("Set up your profile by completing these fields")
        self.label3 = QLabel("UserName")
        self.label4 = QLabel("LastName")
        self.label5 = QLabel("FirstName")
        self.label6 = QLabel("Champs incomplet")
        self.label6.hide()

        self.edit1 = QLineEdit()
        self.edit2 = QLineEdit()
        self.edit3 = QLineEdit()
        
        self.button = QPushButton("Submit")

        # Create layout and add widgets

        layoutVer1 = QVBoxLayout()

        layoutVer1.addWidget(self.label1)
        layoutVer1.addWidget(self.label2)

        layoutVer1.addWidget(self.label3)
        layoutVer1.addWidget(self.edit1)
        layoutVer1.addWidget(self.label4)
        layoutVer1.addWidget(self.edit2)
        layoutVer1.addWidget(self.label5)
        layoutVer1.addWidget(self.edit3)
        layoutVer1.addWidget(self.label6)

        layoutVer1.addWidget(self.button)

        # Set dialog layout
        self.setLayout(layoutVer1)

        # Add button signal to greetings slot
        self.button.clicked.connect(self.Submit)

    # Greets the user
    def Submit(self):

        if (self.edit1.text()=="" or self.edit2.text()=="" or self.edit3.text()==""):
            
            self.edit1.setText("")
            self.edit2.setText("") 
            self.edit3.setText("")
            self.label6.show()

        else:

            conn = sqlite3.connect("important.db")
            c = conn.cursor()

            c.execute("SELECT id FROM personne ORDER BY id DESC LIMIT 1")
            result = c.fetchone()
            result=str(result)
            lastid=""
            for i in range(len(result)):
                if result[i].isdigit():
                    lastid+=result[i]
                else:
                    pass

            c.execute("""UPDATE personne SET nom = ?, prenom = ? WHERE username = ? AND id = ?""", (self.edit2.text(), self.edit3.text(), self.edit1.text(), lastid))
            
            form.show()
            form5.hide()
            self.label6.hide()

            conn.commit()
            conn.close()

            self.edit1.setText("")
            self.edit2.setText("") 
            self.edit3.setText("")

class Window(QDialog):

    def __init__(self, parent=None):
        super(Window, self).__init__(parent)
        # Create widgets

        self.setWindowTitle('Secret Window')
        self.label1 = QLabel("Welcome")
        self.label2 = QLabel("Ultra High Secret window")

        self.button = QPushButton("Open website")
        self.button2 = QPushButton("Exit")

        # Create layout and add widgets

        layoutVer = QVBoxLayout()

        layoutVer.addWidget(self.label1)
        layoutVer.addWidget(self.label2)
        layoutVer.addWidget(self.button)
        layoutVer.addWidget(self.button2)

        # Set dialog layout
        self.setLayout(layoutVer)

        # Add button signal to greetings slot
        self.button.clicked.connect(self.Open)
        self.button2.clicked.connect(self.Exit)

    # Greets the user
    def Open(self):

        webbrowser.open("https://snirpy.github.io/site/")

    def Exit(self):

        form.show()
        form3.hide()

if __name__ == '__main__':
    # Create the Qt Application
    app = QApplication(sys.argv)
    # Create and show the form
    form = Connexion()
    form2 = Login()
    form3 = Window()
    form4 = SignUp()
    form5 = Submit()
    form.show()
    form2.hide()
    form3.hide()
    form4.hide()
    form5.hide()
    # Run the main Qt loop
    sys.exit(app.exec())
